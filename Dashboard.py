...
classifier = load_model('VSLModel.h5')			#loading the model

# Custom queue to check for last X elements are the same
class TextQueue:
	data = None
	...
	def isStable(self):
		if len(self.data) != self.size:
			...
			val = self.data[0]
			return all(p == val for p in self.data)
...
def predictor():
	...
	# TODO: Change return value below from letter to desired word. Abuses existing alphabet
	# Check template.png for corresponding gestures
	if result[0][0] == 1:
		  return 'am' # Originally A
	elif result[0][1] == 1:
		  return 'boring' # Originally B
	elif result[0][2] == 1:
		  return 'clean' # Originally C
	elif result[0][3] == 1:
		  return 'help' # Originally D
	elif result[0][4] == 1:
		  return 'house' # Originally E
	elif result[0][5] == 1:
		  return 'I' # Originally F
	elif result[0][6] == 1:
		  return 'play' # Originally G
	elif result[0][7] == 1:
		  return 'soccer'
	elif result[0][8] == 1:
		  return 'you'
	else:
		# None matches, defaults to space
		return 'Space'

def reformat(text):
	# TODO: Space rendering, underscore because the current UI does not support cursor. Change after else
	return text if text != 'Space' else '_'
...
class Dashboard(QtWidgets.QMainWindow):
	...
	def scanSent(self):
		...
		finalBuffer=[]
		counts=0
		# TODO: change number in TextQueue constructor to the desired number of frames
		#  			that are required to be stable for gesture capturing
		textQueue = TextQueue(20)
		countdown = 0
		prevWord = None
		while True:
			if countdown > 0:
			...
			cv2.imwrite(img_name, save_img)
			img_text=predictor()
			textQueue.append(img_text);
			if textQueue.isStable():
					try:
						counts+=1

						# TODO: If similar gestures need to be recognized consecutively
						# remove this
						# begin remove
						if prevWord == img_text:
							# Same gesture as the previous word
							# Append space instead
							img_text = 'Space'
						else:
							prevWord = img_text
						# end remove
						...
						# TODO: Change countdown to desired number of non-recording 'frames' after recognizing a gesture
						countdown = 100
